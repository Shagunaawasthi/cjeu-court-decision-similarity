    ...
    for file in f:
        if '.txt' in file:
            ...
            with open (path+file, "r", encoding="utf-8") as myfile:
                ...
                value_to_celex[data] = celexnum
                tknzd = tokenize(data)
                value_to_tokenized[data] = tknzd
...
def lookup_similar_cases(sample_cases, n, topic):
    ...
    for item in sample_cases:
        ...
        for reference in topn:
            results.append([item,reference[0],reference[1],'jaccard',exists_citation_link_between(item,reference[0]),topic])
...
if os.path.exists('../outputdata/results_jaccard.csv') == False:
    results.insert(0,['source_case','similar_case','similarity_score','method','citation_link','source_case_topic'])
    
with open('../outputdata/results_jaccard.csv', 'a', newline='') as outfile:
