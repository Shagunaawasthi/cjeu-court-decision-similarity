...
tfidfvect = TfidfVectorizer(analyzer='word', ngram_range=(3,3), use_idf=True, stop_words=stopwords_full)
...
pickle.dump(tfidf_data, open("tfidfmodel_trigram.pickle", "wb"))
...
def lookup_similar_cases(sample_cases, n, topic):
    ...
    for item in sample_cases:
        ...
        for reference in similar_cases_references:
            results.append([item,reference[0],reference[1],'tfidf-trigram',exists_citation_link_between(item,reference[0]),topic])
...
if os.path.exists('../outputdata/results_trigram.csv') == False:
    results.insert(0,['source_case','similar_case','similarity_score','method','citation_link','source_case_topic'])
    
with open('../outputdata/results_trigram.csv', 'a', newline='') as outfile:
